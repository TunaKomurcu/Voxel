...
## Layer 2.5 — Text-only self-play (persona consistency)

`tests/self_play.py` runs each real persona (`agents/*.jsonc`, unmodified)
against a set of synthetic "founder" behavior profiles, both sides
generated by the LLM Gateway (qwen3.5-4b-32k-fast) — no Voice Agent API,
no audio. Cheap ($0.03 for a 4-persona x 10-archetype x 5-turn batch) and
fast to iterate with, but for that same reason it only tests **persona
consistency and character behavior** — it produces no real audio
timestamps, so it says nothing about actual barge-in/turn-detection
behavior (that still needs a Layer 3 live call).

First full run (2026-09-16, 40 combinations: 4 personas x 10 founder
archetypes) caught two real, since-fixed prompt gaps: Marcus and Priya
never once softened tone across 5 turns of a founder giving genuinely
strong, verifiable answers (the prompt's own "acknowledge briefly" line
had no teeth), and Marcus attributed claims to the founder that were
never actually said (an unstructured version of the jargon-specific
"never invent" rule Priya already had). Both fixed by strengthening the
relevant prompt lines across all four `agents/*.jsonc` files; re-run
confirmed on Grace and Derek (explicit "Okay, that's a real number" /
"Okay, that actually makes sense" acknowledgments now appear, and
disappear again once the founder's answers stop being genuinely clear —
see the `grace__strong_control_plain.json` and `derek__*.json` transcripts
under `tests/self_play_transcripts/`).

Two methodological limits surfaced by that run, worth knowing before
trusting a transcript at face value:

1. **The founder simulator (qwen3.5-4b, the cheap model) occasionally
   produces nested hallucinations and language-mixing** — e.g. inventing
   a fake mini-exchange inside its own single turn
   (`derek__monosyllabic.json`: one founder turn contained fabricated
   "Derek:" lines of its own), and stray non-English character fragments
   showing up mid-sentence in both founder and persona turns
   (`derek__overconfident_fabricator.json`, `derek__self_correcting.json`).
   This is a small-model reliability limit, not a persona-prompt issue —
   don't mistake founder-side gibberish for a real persona bug when
   reading self-play transcripts.
2. **`max_tokens=80` (the script's per-line budget) sometimes truncates a
   reply mid-word or mid-number**, especially the `overconfident_fabricator`
   archetype's number-dense style. A script limitation, not a signal about
   the persona reacting to it.

The generic `strong_control` archetype (a technically-detailed, jargon-heavy
"well-prepared founder") also turned out to be a poor fit for testing
Grace specifically — jargon-heavy is never a "genuinely clear,
plain-language answer" by *her* own definition, so it never actually
exercised her softening rule. Added `strong_control_plain` (concrete,
numbered, but deliberately jargon-free) as her equivalent control case.
Keep this in mind when adding a new persona whose definition of "a good
answer" differs from the existing archetypes' assumptions — it may need
its own control-case variant rather than reusing the generic one.
